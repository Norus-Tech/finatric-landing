    ...
    render() {
        ...
        return (
            <Link to={withPrefix(_.get(action, 'url', null))}
            	...
            	{...((_.get(action, 'new_wind
            	{_.get(action, 'has_icon', null) ? (<React.Fragment>ow', null) || _.get(action, 'no_follow', null)) ? ({rel: (_.get(action, 'new_window', null) ? ('noopener ') : '') + (_.get(action, 'no_follow', null) ? ('nofollow') : '')}) : null)}
